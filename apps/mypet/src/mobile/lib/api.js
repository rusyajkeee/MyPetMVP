...
const CATEGORY_SYNONYMS = [
  {
    ...
    synonyms: [
      // Русский
      'груминг', 'стрижка', 'подстричь', 'постричь', 'мытье', 'мытьё', 'помыть', 'вымыть',
      'мойка', 'мою', 'моют', 'купание', 'искупать', 'купать', 'купаю',
      'шерсть', 'уход за шерстью', 'расчесывание', 'расчесать', 'расчёска', 'вычесывание', 'вычесать',
      'тримминг', 'триммер', 'линька', 'колтуны', 'колтун',
      'когти', 'коготь', 'стрижка когтей', 'подрезать когти', 'обрезать когти', 'когтерез',
      'уши', 'чистка ушей', 'чистить уши',
      'зубы', 'чистка зубов', 'чистить зубы', 'зубной камень',
      'шампунь', 'кондиционер для шерсти', 'расчёска',
      'салон', 'груминг-салон', 'зоосалон', 'парикмахер', 'парикмахерская',
      'спа', 'spa', 'красота', 'внешний вид',
      'лапы', 'лапки', 'подушечки лап',
      'хвост', 'уши торчком',
      // Казахский
      'тазалау', 'шомылдыру', 'тырнақ', 'жүн', 'шаштараз',
      // Английский
      'groom', 'grooming', 'bath', 'wash', 'trim', 'clip', 'nails', 'fur', 'coat',
      'brushing', 'deshed', 'de-shed', 'blowdry',
    ],
  },
  {
    slug: 'VETERINARY',
    synonyms: [
      // Русский — врач/клиника
      'ветеринар', 'ветклиника', 'ветеринарная клиника', 'вет', 'ветврач', 'ветеринарный',
      'врач', 'доктор', 'клиника', 'больница', 'кабинет', 'центр',
      // Лечение
      'лечение', 'лечить', 'лечиться', 'заболел', 'заболела', 'болеет', 'болезнь', 'больной',
      'не ест', 'вялый', 'температура', 'рвота', 'понос', 'кашель', 'чихает',
      // Прием
      'прием', 'приём', 'записаться', 'записать', 'запись', 'осмотр', 'консультация',
      // Вакцинация
      'вакцина', 'вакцинация', 'прививка', 'прививки', 'сделать прививку', 'вакцинировать',
      'бешенство', 'чипирование', 'чип', 'паспорт животного',
      // Анализы и диагностика
      'анализ', 'анализы', 'кровь', 'узи', 'рентген', 'диагностика', 'мрт', 'экг',
      // Операции
      'хирург', 'операция', 'хирургия', 'стерилизация', 'стерилизовать', 'кастрация',
      'кастрировать', 'удаление', 'зашить', 'рана',
      // Другое
      'терапевт', 'укол', 'капельница', 'антибиотик', 'лекарство', 'таблетки',
      'блохи', 'клещи', 'глисты', 'паразиты', 'обработка',
      'скорая', 'срочно', 'экстренно', 'ночной ветеринар',
      // Казахский
      'мал дәрігері', 'емхана', 'егу', 'дәрі',
      // Английский
      'vet', 'veterinary', 'clinic', 'doctor', 'vaccine', 'vaccination', 'surgery',
      'diagnosis', 'treatment', 'checkup', 'exam', 'xray', 'ultrasound',
    ],
  },
  {
    slug: 'BOARDING',
    synonyms: [
      // Русский
      'передержка', 'гостиница', 'зоогостиница', 'отель', 'отель для животных',
      'отель для собак', 'отель для кошек', 'приют временный',
      'присмотр', 'оставить питомца', 'на время отпуска', 'на время поездки',
      'уехать', 'командировка', 'с кем оставить', 'некому оставить',
      'дневной уход', 'ночевка', 'ночёвка', 'ночевать',
      'постой', 'разместить', 'поселить',
      // Казахский
      'асыраушы', 'уақытша',
      // Английский
      'boarding', 'daycare', 'pet hotel', 'kennel', 'overnight', 'sitting', 'pet sitting',
    ],
  },
  {
    slug: 'TRAINING',
    synonyms: [
      // Русский
      'дрессировка', 'дрессировать', 'дрессировщик', 'дрессир',
      'тренировка', 'тренер', 'тренировать',
      'воспитание', 'воспитать', 'воспитывать',
      'команды', 'команда', 'послушание', 'послушный',
      'поведение', 'агрессия', 'агрессивный', 'кусается', 'лает',
      'обучение', 'научить', 'обучить',
      'кинолог', 'зоопсихолог', 'психолог',
      'нюх', 'нюхач', 'охота', 'охотничий',
      'щенок обучение', 'социализация',
      // Казахский
      'үйрету', 'жаттықтыру',
      // Английский
      'training', 'trainer', 'obedience', 'behavior', 'behaviorist',
      'command', 'teach', 'puppy class',
    ],
  },
  {
    slug: 'SHELTER',
    synonyms: [
      // Русский
      'приют', 'зооприют', 'усыновление', 'взять питомца', 'взять собаку', 'взять кошку',
      'питомник', 'спасение', 'бездомный', 'бездомная', 'бродячий',
      'найти хозяина', 'ищет дом', 'ищет хозяина', 'в добрые руки', 'отдам',
      'подобрал', 'подобрала', 'пристрою',
      // Казахский
      'баспана', 'асырап алу',
      // Английский
      'shelter', 'rescue', 'adoption', 'adopt', 'foster', 'stray', 'homeless pet',
    ],
  },
];
